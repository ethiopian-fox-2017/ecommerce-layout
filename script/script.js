$(document).ready(()=> {
  ...
  $('.add-to-cart').on('click', (e)=> {
    console.log($(e.target).siblings('#price').children().text())
    let phoneName = $(e.target).siblings('#phone-name').text(),
        price = $(e.target).siblings('#price').children().text().split('.').join(),
        append = $(`<div class="panel-block">${phoneName} Rp. ${price}</div>`)
